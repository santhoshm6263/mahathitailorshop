...
// src/features/BlouseCustomizer/index.tsx
...
export const BlouseCustomizer: React.FC<BlouseCustomizerProps> = ({ 
  onSuccess,
  setCurrentPage 
}) => {
  const { addOrder } = useBoutique();
  ...
  const prices = {
    neck: { 'Round Neck': 0, 'Royal Pot Neck': 400, 'Boat Neck': 300, 'Sweetheart Neck': 500, 'Queen Anne': 600 } as Record<string, number>,
    sleeve: { 'Short Sleeve': 0, 'Elbow Sleeve': 200, 'Full Sleeve': 400, 'Puff Sleeve': 300 } as Record<string, number>,
    embroidery: { 'Minimalist Border': 2500, 'Medium Stone Work': 4500, 'Heavy Aari & Zardosi': 7500, 'Royal Maggam Work': 9500 } as Record<string, number>
  };
  ...
  const handleSubmitDesign = async (e: React.FormEvent) => {
    ...
    const orderData = {
      ...
      phone: customerPhone,
      stitching_item: 'Custom Designer Blouse',
      neck_style: selections.neckStyle,
      sleeve_style: selections.sleeveStyle,
      embroidery_style: selections.embroideryStyle,
      fabric_notes: `Fabric Type: ${selections.fabricType}. Custom requests: ${selections.customNotes}. Tassels: ${selections.tasselsRequested ? 'Yes' : 'No'}. Padding: ${selections.paddingRequested ? 'Yes' : 'No'}`,
      cost: totalCost,
      advance_paid: 0,
      due_date: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 14 days from now
      inspiration_image: uploadedImage || '/assets/designer-blouse.png',
      staff_assigned: 'Pending Assignment'
    };
    ...
    try {
      ...
      alert('Failed to submit design order. Please check connection and try again.');
    }
  };
  ...
  return (
    <div className="w-full bg-white rounded-3xl border border-[#5C061E]/5 shadow-luxury overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
        ...
        <div className="lg:col-span-8 p-6 sm:p-10 border-r border-[#5C061E]/5 max-h-[70vh] overflow-y-auto">
                  ...
                  <div 
                    ...
                    className={`p-5 rounded-2xl border-2 transition-all duration-300 cursor-pointer flex flex-col justify-between ${
                      selections.neckStyle === opt.name 
                        ...
                        : 'border-[#5C061E]/5 bg-[#FFF8F0]/10 hover:bg-[#FFF8F0]/40'
                    }`}
                  >
                    <div>
          ...
          {step === 4 && (
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-[#FFF8F0]/40 border border-[#5C061E]/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-gold transition-colors duration-300"
                    required
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-[#FFF8F0]/40 border border-[#5C061E]/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-gold"
                  >
              ...
              <div className="space-y-3">
                ...
                <div className="border-2 border-dashed border-[#5C061E]/20 hover:border-gold rounded-2xl p-6 flex flex-col items-center justify-center bg-[#FFF8F0]/10 transition-colors duration-300 relative">
                  {uploadedImage ? (
                    <div className="relative w-28 h-28 rounded-xl overflow-hidden border border-gold/30">
                      ...
                      <button 
                        ...
                        className="absolute inset-0 bg-black/60 flex items-center justify-center text-white text-xs font-bold font-poppins cursor-pointer"
                      >
                        Remove
                      </button>
                    </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-[#FFF8F0]/40 border border-[#5C061E]/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-gold h-20"
                ></textarea>
              </div>
          ...
          <div className="flex items-center justify-between border-t border-[#5C061E]/5 pt-8 mt-8">
            <button 
              type="button"
              onClick={() => setStep(prev => Math.max(1, prev - 1))}
              disabled={step === 1}
              className="font-poppins text-xs uppercase tracking-widest font-semibold text-maroon/40 disabled:opacity-30 px-6 py-2 cursor-pointer"
            >
              ← Back
            </button>
            ...
            {step < 4 ? (
              <button 
                ...
                className="font-poppins text-xs uppercase tracking-widest font-semibold bg-maroon hover:bg-gold text-gold hover:text-maroon px-8 py-3 rounded-full transition-all duration-300 cursor-pointer"
              >
                Next Step →
              </button>
            ) : (
              <button 
                ...
                className="font-poppins text-xs uppercase tracking-widest font-bold bg-gold hover:bg-white text-maroon px-8 py-3 rounded-full border border-gold transition-all duration-300 cursor-pointer"
              >
        ...
        <div className="lg:col-span-4 bg-[#FFF8F0]/40 p-6 sm:p-10 flex flex-col justify-between">
          
          <div className="space-y-6">
            <h4 className="font-playfair text-xl font-bold text-maroon pb-4 border-b border-[#5C061E]/10">
              Live Custom Summary
            </h4>
            ...
            <div className="space-y-4 font-poppins text-xs">
              ...
              <div className="flex items-start justify-between border-t border-[#5C061E]/5 pt-4">
            ...
            <div className="border border-gold/15 bg-white rounded-2xl p-4 shadow-luxury flex items-center justify-center h-[180px] relative overflow-hidden">
              <div className="absolute inset-0 opacity-10 bg-[radial-gradient(#5C061E_1px,transparent_1px)] [background-size:12px_12px]"></div>
              
              {uploadedImage ? (
                <img src={uploadedImage} className="w-full h-full object-cover rounded-xl" />
              ) : (
                <div className="text-center z-10 space-y-2">
                  <span className="text-3xl block animate-pulse-soft">👑</span>
          ...
          <div className="border-t border-[#5C061E]/10 pt-6 mt-8">
      ...
      {orderCompleteModal && (
        <div className="fixed inset-0 bg-maroon-dark/95 z-50 flex items-center justify-center p-4 sm:p-8 animate-fade-in">
          <div className="bg-white rounded-3xl p-8 max-w-lg w-full text-center border border-gold/20 shadow-glow space-y-6">
            ...
            <div className="w-16 h-16 rounded-full bg-gold/10 text-gold border border-gold/30 flex items-center justify-center text-3xl mx-auto">
              ✔
            </div>
            ...
            <div className="w-full border-t border-[#5C061E]/5 pt-4 space-y-3 font-poppins text-xs text-maroon/70 text-left">
            ...
            <div className="space-y-3 pt-4">
              <button 
                ...
                className="w-full font-poppins text-xs uppercase tracking-widest font-bold bg-gold hover:bg-gold-dark text-maroon py-3 rounded-full border border-gold transition-colors duration-300 cursor-pointer"
              >
                Send Design to Shop on WhatsApp
              </button>
              <button 
                onClick={() => { setOrderCompleteModal(null); if (setCurrentPage) setCurrentPage('home'); }}
                className="w-full font-poppins text-xs uppercase tracking-widest font-semibold bg-transparent hover:bg-maroon/5 text-maroon py-3 cursor-pointer"
              >
                Return to Homepage
              </button>
            </div>
