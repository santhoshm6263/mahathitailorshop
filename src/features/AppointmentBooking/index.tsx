...
// src/features/AppointmentBooking/index.tsx
import React, { useState, useEffect } from 'react';
import { useBoutique } from '../../app/providers';
import { Appointment, AppointmentType } from '../../shared/types';
...
export const AppointmentBooking: React.FC<AppointmentBookingProps> = ({ 
  ...
  const { appointments, addAppointment } = useBoutique();
  ...
  const [formData, setFormData] = useState({
    ...
    date: '',
    timeSlot: '',
    type: 'Bridal Consultation' as AppointmentType,
    notes: ''
  });
  ...
  useEffect(() => {
    if (preSelectedService) {
      setFormData(prev => ({ 
        ...prev, 
        type: preSelectedService.toLowerCase().includes('bridal') ? 'Bridal Consultation' : 'Boutique Visit',
  ...
  const isSlotBooked = (date: string, slot: string) => {
    ...
    return appointments.some(
      apt => apt.date === date && apt.time === slot && apt.status !== 'Cancelled'
    );
  };
  ...
  return (
    <div className="w-full bg-[#FFF8F0]/30 rounded-3xl border border-[#5C061E]/5 shadow-luxury overflow-hidden p-6 sm:p-10">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <form onSubmit={handleSubmitBooking} className="lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-white border border-[#5C061E]/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-gold transition-colors"
                required
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-white border border-[#5C061E]/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-gold"
                required
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-white border border-[#5C061E]/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-gold text-maroon font-semibold"
                required
              />
            </div>
          ...
          <div>
            ...
            {!formData.date ? (
              <p className="font-poppins text-xs text-maroon/40 italic">Please select a date first to view slot availability.</p>
            ) : (
                      ...
                      className={`p-3.5 rounded-xl border text-center font-poppins text-xs font-semibold cursor-pointer transition-all duration-300 ${
                        booked 
                          ...
                          : formData.timeSlot === slot
                            ...
                            : 'bg-white border-[#5C061E]/10 hover:border-gold text-maroon hover:bg-[#FFF8F0]/30'
                      }`}
                    >
                      <span>{slot}</span>
                      {booked && <span className="block text-[8px] tracking-wide font-normal uppercase text-maroon/40 mt-1">Booked</span>}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="pt-4">
            <button
              type="submit"
              className="font-poppins text-xs uppercase tracking-widest font-bold bg-maroon hover:bg-gold text-gold hover:text-maroon py-4 px-8 rounded-full border border-maroon hover:border-gold transition-all duration-300 w-full sm:w-auto cursor-pointer"
            >
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div className="space-y-4 font-poppins text-xs">
              <div className="flex items-start space-x-3">
                ...
                <span className="leading-relaxed">Beside of MMP Primary School, Ponnangur, Adavibhudhugur Road, Mallanur, Kuppam, Chittoor, Andhra Pradesh - 517425</span>
              </div>
              <div className="flex items-start space-x-3">
                <span className="text-gold">📞</span>
                <span>+91 93470 01303</span>
              </div>
              <div className="flex items-start space-x-3">
                <span className="text-gold">⏰</span>
                <span>Welcoming you full week, Monday to Sunday, (9am - 9pm) Sunup to Sundown</span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-lg w-full text-center border border-gold/20 shadow-glow space-y-6">
            ...
            <div className="space-y-3 pt-4">
              <button 
                ...
                className="w-full font-poppins text-xs uppercase tracking-widest font-bold bg-gold hover:bg-gold-dark text-maroon py-3.5 rounded-full border border-gold transition-colors duration-300 cursor-pointer"
              >
